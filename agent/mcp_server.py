#!/usr/bin/env python3
"""VMaxxing MCP Server — expose finance research tools to any MCP client.
...
trading-connector reads, swarm orchestration, trade-journal
...
mcp = FastMCP("VMaxxing", version=APP_VERSION)
...
def add_goal_evidence(
    ...
    Args:
        ...
        run_id: VMaxxing run id. It verifies evidence only when the run directory exists.
...
def main():
    """Entry point for `vmx-mcp` CLI command."""
    global _include_shell_tools, _registry
    import argparse

    parser = argparse.ArgumentParser(description="VMaxxing MCP Server")
    parser.add_argument(
        "--transport",
