...
"vmaxxing[mt5]"``), and a running, logged-in MT5 terminal — when any of
those are missing :meth:`DataLoader.is_available` is ``False`` and the forex
fallback chain degrades to akshare/yfinance/local.

Config is shared with the MT5 trading connector via ``~/.vmx/mt5.json``
...
_MT5_CONFIG_PATH = Path.home() / ".vmx" / "mt5.json"
