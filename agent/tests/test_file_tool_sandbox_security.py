...
def test_tilde_expansion_resolves_to_mock_home(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("VIBE_TRADING_ALLOWED_WRITE_ROOTS", str(mock_home / ".vmx"))
    ...
    resolved = resolve_safe_path("~/.vmx/scripts/strat.py", None, allowed_write, purpose="write")
    assert resolved == mock_home / ".vmx" / "scripts" / "strat.py"
