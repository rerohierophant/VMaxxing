...
from src.providers.openai_compat import AIMessage
...
def test_dict_history_replay_preserves_thought_signature() -> None:
    ...
    # The assistant message dict is replayed verbatim to the OpenAI-compatible
    # endpoint, so both the normalized ``tool_calls`` view and the raw
    # ``additional_kwargs`` payload must carry the signature.
    raw_tool_call = message["additional_kwargs"]["tool_calls"][0]
