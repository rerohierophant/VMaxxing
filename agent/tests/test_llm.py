...
from src.providers.llm import OpenAICompatClient, _sync_provider_env, build_llm
...
def test_build_anthropic_routes_through_openai_compat_client() -> None:
    """build_llm() routes Anthropic through the OpenAI-compatible client.

    The former langchain-anthropic native adapter is gone; Anthropic is reached
    via the OpenAI-compatible client pointed at the Anthropic base URL.
    """
    import src.providers.llm as llm_mod
    ...
    assert isinstance(result, OpenAICompatClient)
    assert result._model == "claude-sonnet-4-6[1M]"
    assert result._api_key == "PROXY_MANAGED"
    assert result._base_url == "http://host.docker.internal:15721"
