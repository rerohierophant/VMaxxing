...
2. MCP server mode regression — importing mcp_server.py must not raise, and
   ...
   confirms that the server-side MCP plugin (vmx-mcp) is unaffected
...
- TODO(v1): Add a live smoke test that spawns `vmx-mcp` as a stdio
...
# ---------------------------------------------------------------------------
# MCP server mode regression (vmx-mcp plugin)
# ---------------------------------------------------------------------------
