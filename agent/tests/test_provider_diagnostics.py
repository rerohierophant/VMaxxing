...
from src.providers.llm import OpenAICompatClient, build_llm, provider_diagnostics
...
def test_provider_capabilities_are_provider_specific() -> None:
    ...
    assert kimi.default_headers["User-Agent"].startswith("VMaxxing/")
...
def test_reasoning_effort_extra_body_is_openrouter_only() -> None:
    ...
    with patch.dict(os.environ, env, clear=True):
        with patch.object(llm_mod, "OpenAICompatClient", _FakeChatOpenAI):
            build_llm()
...
def test_kimi_user_agent_header_is_moonshot_only() -> None:
    ...
    assert captured["default_headers"]["User-Agent"].startswith("VMaxxing/")
...
def test_nvidia_provider_uses_bearer_auth_with_compatibility_user_agent() -> None:
    ...
    assert nvidia.default_headers["User-Agent"].startswith("VMaxxing/")
...
def test_build_llm_returns_openai_compat_client_for_deepseek(monkeypatch) -> None:
    """build_llm() routes DeepSeek through the OpenAI-compatible client.

    The former langchain-deepseek native adapter is gone; DeepSeek is reached
    ...
    # DeepSeek captures reasoning but must NOT echo reasoning_content back.
    assert llm._caps.capture_reasoning is True
    assert llm._caps.send_reasoning_content is False
