"""Interactive CLI front door for VMaxxing.
...
The console script entry in ``pyproject.toml`` (``vmx = "cli:main"``)
hits :func:`main`.
"""
...
def _probe_session_count() -> int:
    ...
    db_path = Path.home() / ".vmx" / "sessions.db"
...
def _show_banner() -> None:
    ...
    console.print(
        ...
        "[/dim][bold]vmx connector --help[/bold][dim] · "
...
class InteractiveContext:
    ...
    Attributes:
        ...
        pending_prompt: Optional prompt queued by a slash handler
            (``/journal``) that the loop should execute as
...
def _new_session(prompt_preview: str) -> Optional[str]:
    ...
    (``~/.vmx/sessions.db``) so cross-session search via
...
def _run_one_turn(user_input: str, ctx: InteractiveContext) -> None:
    ...
    try:
        ...
        dashboard.close()
        # BrokenPipe: caller did ``vmx chat | head`` and the
...
def _trip_halt_from_repl(console: Any, *, reason: str) -> None:
    ...
    console.print(
        ...
        "disabled until you run [bold]/resume[/bold] or [bold]vmx connector resume[/bold]."
    )
...
def _clear_halt_from_repl(console: Any) -> None:
    ...
    path so the model never performs it. Mirrors ``vmx connector resume``
...
def _run_connector_command_from_repl(console: Any, args: list[str]) -> None:
    ...
    ``/connector`` is a thin in-REPL bridge to the ``vmx connector``
...
def _handle_proposal_reply(text: str, ctx: InteractiveContext) -> bool:
    ...
    # Live mandate commit requires the web surface (api_server), which has been
    # removed for the research-only build. Inform the user; the proposal stays
    # open so they can adjust instead of committing.
    console.print(
        "[yellow]Mandate commit is disabled in research-only mode[/yellow] "
        "(the live execution API was removed). The proposal remains open — "
        "type an adjust sentence to refine it."
    )
    return True
...
def _interactive_loop(max_iter: int, resume_session_id: Optional[str] = None) -> int:
    ...
    if resume_session_id:
        # Resume a specific session by ID (``vmx resume <session-id>``).
        try:
            store = _session_store()
    ...
    while True:
        ...
        if text.startswith("/"):
            ...
            # Drain it here so the slash command turns
    ...
    if ctx.session_id:
        console.print(
            f"[dim]To resume this session:[/dim] [bold]vmx resume {ctx.session_id}[/bold]"
        )
    return 0
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    * Non-interactive entry (``serve``, ``run -p ...``, ``mcp``,
    ...
    # Handle ``vmx resume <session-id>`` — enter the interactive
...
def _build_typer_app():  # type: ignore[no-untyped-def]
    ...
    app = typer.Typer(
        ...
        help="VMaxxing — natural-language finance research agent.",
