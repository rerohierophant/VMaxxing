#!/usr/bin/env python3
"""VMaxxing CLI for natural-language finance research and backtesting.

Usage:
    vmx                           Interactive mode (default)
    vmx -p "Backtest AAPL MACD"   Single run
    vmx serve --port 8899         Start API server
    vmx chat                      Interactive mode
    vmx list                      List runs
    vmx show <run_id>             Show run details
"""
...
class _RunDashboard:
    ...
    def render(self) -> Panel:
        ...
        return Panel(body, title="VMaxxing", border_style="cyan", padding=(1, 1 if compact else 2))
...
def _print_result(result: dict, elapsed: float, *, no_rich: bool = False) -> None:
    ...
    if result.get("run_id"):
        ...
        actions.add_row(f"vmx show {rid}", "details")
        actions.add_row(f"vmx code {rid}", "generated Python")
        actions.add_row(f"vmx continue {rid} \"...\"", "refine this run")
...
def _build_welcome_panel(term_width: Optional[int] = None) -> Panel:
    ...
    credential_ready = provider == "ollama" or bool(key_value)
    ...
    return Panel(body, title="[bold cyan]VMaxxing[/bold cyan]", border_style="cyan", padding=(1, 1))
...
def _show_settings() -> None:
    ...
    if compact:
        ...
        console.print(Columns(panels, expand=True, equal=True))
    console.print("[dim]Edit configuration in ~/.vmx/.env, or run vmx init.[/dim]")
...
def cmd_provider_login(provider: str) -> int:
    ...
    console.print(
        "[red]OAuth providers are not supported in this "
        "OpenAI-compatible build.[/red] Use an OpenAI-compatible provider "
        "(e.g. openai / deepseek) configured via LANGCHAIN_PROVIDER, "
        "OPENAI_API_KEY, OPENAI_BASE_URL, and LANGCHAIN_MODEL_NAME."
    )
    return EXIT_USAGE_ERROR
...
# widen a mandate). The public CLI surface is `vmx connector ...`;
...
def _print_channels_status(payload: Dict[str, Any]) -> None:
    ...
    if payload.get("status") == "error":
        ...
        console.print("[dim]Start the backend with `vmx serve --port 8000`, or inspect local config with this status output.[/dim]")
...
def cmd_channels_start(*, json_mode: bool = False) -> int:
    ...
    if json_mode:
        ...
        console.print(
            "[dim]Run `vmx serve --port 8000` first, or set VIBE_TRADING_API_URL.[/dim]"
        )
    else:
...
def cmd_channels_login(channel_name: str, *, force: bool = False) -> int:
    ...
    if channel_name == "websocket":
        ...
        console.print("[dim]Configure channels.websocket in ~/.vmx/agent.json, then run `vmx channels start`.[/dim]")
        return EXIT_SUCCESS
    if not section:
        console.print(f"[red]No config found for channel '{channel_name}'.[/red]")
        console.print("[dim]Add it under channels.<name> in ~/.vmx/agent.json, then retry.[/dim]")
        return EXIT_USAGE_ERROR
...
def _dispatch_channels(args: argparse.Namespace) -> int:
    ...
    console.print("[red]channels requires a subcommand.[/red] Try: vmx channels status")
    return EXIT_USAGE_ERROR
# QVERIS-INTEGRATION
...
def _dispatch_data(args: argparse.Namespace) -> int:  # QVERIS-INTEGRATION
    ...
    console.print("[red]data requires a subcommand.[/red] Try: vmx data status")  # QVERIS-INTEGRATION
    return EXIT_USAGE_ERROR  # QVERIS-INTEGRATION
# QVERIS-INTEGRATION
...
def _print_missing_live_channel_config(key: str) -> None:
    ...
    console.print(
        ...
        "Add the broker's mcpServers entry to ~/.vmx/agent.json first."
    )
...
def cmd_live_authorize(broker: str) -> int:
    ...
    console.print(
        ...
        "order tools. Use `vmx connector status` to check state.[/dim]"
    )
    return EXIT_SUCCESS
...
def cmd_live_halt(broker: Optional[str] = None) -> int:
    ...
    the switch is cleared with ``vmx connector resume``.
    ...
    console.print("[dim]Run `vmx connector resume` to re-enable.[/dim]")
    return EXIT_SUCCESS
...
def cmd_live_start(broker: Optional[str] = None) -> int:
    ...
    if result.get("status") == "error":
        ...
        console.print(
            "[dim]Is the API server running? Start it with `vmx serve`.[/dim]"
        )
        return EXIT_RUN_FAILED
    ...
    console.print("[dim]Check it with `vmx connector status`.[/dim]")
    return EXIT_SUCCESS
...
def cmd_connector_list() -> int:
    ...
    console.print(table)
    console.print("[dim]Use `vmx connector use <profile>` to set the default profile.[/dim]")
    return EXIT_SUCCESS
...
def cmd_connector_configure(
    ...
    *,
    ...
    console.print(f"[dim]Run `vmx connector check {profile.id}` to verify it.[/dim]")
    return EXIT_SUCCESS
...
def _dispatch_connector(args: argparse.Namespace) -> int:
    ...
    console.print("[red]connector requires a subcommand.[/red] Try: vmx connector list")
    return EXIT_USAGE_ERROR


def _dispatch_schedule(args: argparse.Namespace) -> int:
    """Route parsed ``schedule`` subcommands."""
    ...
    if sub == "add":
        return cmd_schedule_add(args.job_id, args.prompt, args.schedule)
    if sub == "delete":
        return cmd_schedule_delete(args.job_id)
    if sub == "pause":
        return cmd_schedule_pause(args.job_id)
    if sub == "resume":
        return cmd_schedule_resume(args.job_id)
    console.print("[red]schedule requires a subcommand.[/red] Try: vmx schedule list")
    return EXIT_USAGE_ERROR


def _dispatch_watch(args: argparse.Namespace) -> int:
    """Start the scheduled-research watch daemon."""
    from cli.watch import cmd_watch

    return cmd_watch(
        tick_interval_ms=args.watch_tick_ms,
        store_path=args.watch_store,
    )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="VMaxxing CLI")
    parser.add_argument("--version", action="version", version=f"vmx {_VERSION}")
    ...
    login_parser.add_argument("provider", help="OAuth provider name (unsupported in OpenAI-compatible build)")
    ...
    channels_login.add_argument("channel_name", help="Channel name, e.g. feishu, email")
    ...
    channels_pairing.add_argument("--channel", default="feishu", help="Channel context for list/revoke commands")
    ...
    schedule_parser = subparsers.add_parser("schedule", help="Manage scheduled research jobs")
    schedule_subparsers = schedule_parser.add_subparsers(dest="schedule_command")
    schedule_subparsers.add_parser("list", help="List scheduled research jobs")

    schedule_add_parser = schedule_subparsers.add_parser("add", help="Add a scheduled research job")
    schedule_add_parser.add_argument("job_id", help="Unique job identifier (slug or UUID)")
    schedule_add_parser.add_argument("prompt", help="Research prompt to execute on schedule")
    schedule_add_parser.add_argument(
        "schedule",
        help="Interval-ms (e.g. 3600000) or 5-field cron (e.g. '0 9 * * 1')",
    )

    schedule_delete_parser = schedule_subparsers.add_parser("delete", help="Delete a scheduled research job")
    schedule_delete_parser.add_argument("job_id", help="Job identifier to remove")

    schedule_pause_parser = schedule_subparsers.add_parser("pause", help="Pause a scheduled research job")
    schedule_pause_parser.add_argument("job_id", help="Job identifier to pause")

    schedule_resume_parser = schedule_subparsers.add_parser("resume", help="Resume a paused scheduled research job")
    schedule_resume_parser.add_argument("job_id", help="Job identifier to resume")

    # Watch daemon
    watch_parser = subparsers.add_parser("watch", help="Start the scheduled-research daemon")
    watch_parser.add_argument(
        "--tick",
        dest="watch_tick_ms",
        type=int,
        default=60_000,
        help="Poll interval in milliseconds (default: 60000 = 60s)",
    )
    watch_parser.add_argument(
        "--store",
        dest="watch_store",
        type=Path,
        default=None,
        help="Explicit store path (default: ~/.vmx/scheduled_research/scheduled_research_jobs.json)",
    )
    ...
    subparsers.add_parser("init", help="Interactive setup: create ~/.vmx/.env")
...
_INIT_ENV_PATH = Path.home() / ".vmx" / ".env"
...
def cmd_memory_show(name: str, *, memory_dir: Optional[Path] = None) -> int:
    ...
    if entry is None:
        ...
        console.print("[dim]Run `vmx memory list` to see available titles.[/dim]")
        return EXIT_USAGE_ERROR
...
def cmd_init() -> int:
    """Interactive setup: create ~/.vmx/.env."""
    console.print(Panel("[bold cyan]VMaxxing setup[/bold cyan]\n[dim]Configure the default LLM provider and data tokens.[/dim]", border_style="cyan"))
    ...
    for idx, option in enumerate(_PROVIDER_CHOICES, start=1):
        credential = "none" if option["key_env"] is None else str(option["key_env"])
    ...
    next_steps.add_row("Run", "[bold]vmx[/bold]")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "provider":
        ...
        console.print("[red]provider requires a subcommand.[/red] Try: vmx provider doctor")
        return EXIT_USAGE_ERROR
    ...
    if args.command == "schedule":
        return _coerce_exit_code(_dispatch_schedule(args))
    if args.command == "watch":
        return _coerce_exit_code(_dispatch_watch(args))
    if args.command == "memory":
        ...
        console.print("[red]memory requires a subcommand.[/red] Try: vmx memory list")
        return EXIT_USAGE_ERROR
