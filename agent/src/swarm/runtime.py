...
class SwarmRuntime:
    ...
    def __init__(
        ...
        """
        self._store = store
        self._max_workers = 1 if os.environ.get("VIBE_SWARM_SERIAL") == "1" else max_workers
    ...
    def _execute_layer(
        ...
        try:
            for tid in layer_task_ids:
                ...
                futures[future] = tid
                _start_times[tid] = time.monotonic()
        ...
        # Benchmark instrumentation: record this layer's parallel vs serial wall.
        # `wall` is what actually happened (tasks overlap within a layer); `serial`
        # is what serial execution would cost (sum of per-task durations). This is
        # the within-run counterfactual for the E4 swarm-concurrency experiment --
        # swarm runs have no trace.jsonl, so the tool-level reader cannot serve it.
        try:
            paired = [
                (s, e)
                for t, e in _end_times.items()
                if (s := _start_times.get(t)) is not None
            ]
            if paired:
                layer_wall = max(e for _, e in paired) - min(s for s, _ in paired)
                layer_serial = sum(e - s for s, e in paired)
                _append_swarm_timing(
                    run_dir, layer_idx, layer_wall, layer_serial, layer_serial - layer_wall, len(paired)
                )
        except Exception:
            logger.warning("swarm layer timing failed", exc_info=True)

        return results

    def _append_swarm_timing(
        self,
        run_dir: Path,
        layer_idx: int,
        wall: float,
        ...
        """Append one layer's parallel/serial timing to ``swarm_timing.jsonl``.

        Read back by bench_agent.py::_read_swarm_timing to build the within-run
        counterfactual for the E4 swarm-concurrency experiment.
        """
        try:
            ...
            logger.warning("failed to write swarm_timing.jsonl", exc_info=True)
