...
User-supplied presets are also discovered from ``~/.vmx/swarm/
...
USER_PRESETS_DIR = Path.home() / ".vmx" / "swarm" / "presets"
...
def load_preset(name: str) -> dict:
    ...
    Raises:
        ...
        FileNotFoundError: If the preset file does not exist in either the
            user directory (``~/.vmx/swarm/presets/``) or the
...
def list_presets() -> list[dict]:
    ...
    User presets (``~/.vmx/swarm/presets/``) are listed alongside the
