...
class ContextBuilder:
    ...
    def build_system_prompt(self, user_message: str = "") -> str:
        ...
        _eager = os.environ.get("VIBE_EAGER_SKILLS", "").strip().lower()
        if _eager and _eager != "0":
            # Benchmark toggle for the progressive-disclosure experiment: inline
            # full skill documentation into the system prompt instead of letting
            # the model pull it on demand via load_skill.
            #
            # "all"/"1" inlines all 87 skills (~264k tokens) -- that exceeds the
            # model context window, which is itself the finding. Pass an integer
            # to inline only the first N skills so the A/B arm stays runnable.
            _limit = None if _eager in ("all", "1") else int(_eager)
            _targets = self.skills_loader.skills[:_limit] if _limit else self.skills_loader.skills
            eager_parts = [skill_descriptions]
            for _skill in _targets:
                # NOTE: iterate Skill objects but pass `.name` -- handing the
                # object itself to get_content() silently degrades to an
                # "Error: Unknown skill" string, collapsing the A/B delta to ~0.
                body = self.skills_loader.get_content(_skill.name)
                if body.startswith("Error:"):
                    raise RuntimeError(
                        f"VIBE_EAGER_SKILLS: cannot inline skill {_skill.name!r}; "
                        "aborting rather than benchmarking a bogus delta."
                    )
                eager_parts.append(body)
            skill_descriptions = "\n\n".join(eager_parts)
