...
def _serial_tools() -> bool:
    """Benchmark toggle: force serial tool execution (disables readonly concurrency)."""
    return os.environ.get("VIBE_SERIAL_TOOLS") == "1"
...
class AgentLoop:
    ...
    def run(self, user_message: str, history: Optional[List[Dict[str, Any]]] = None, session_id: str = "") -> Dict[str, Any]:
        ...
        try:
            while iteration < self.max_iterations:
                ...
                if compact_requested and not _no_compress():
    ...
    def _batch_execute(
        ...
        for mode, batch in batches:
            ...
            if self._cancel_event.is_set():
                break
            if mode == "parallel" and len(batch) > 1 and not _serial_tools():
