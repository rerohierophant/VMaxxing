...
class FeishuChannel(BaseChannel):
    ...
    async def login(self, force: bool = False) -> bool:
        ...
        _LOGIN_CONSOLE.print("Authorize with the mobile app. vmx will save the new bot credentials.\n")

        result = qr_register(initial_domain=self.config.domain or "feishu")
        if not result:
            _LOGIN_CONSOLE.print(
                "[yellow]Login was not completed.[/yellow] "
                "Run 'vmx channels login feishu --force' to retry."
            )
            return False
    ...
    async def start(self) -> None:
        ...
        if not self.config.app_id or not self.config.app_secret:
            self.logger.error(
                ...
                "Run 'vmx channels login feishu' to set up via QR code."
            )
            return
