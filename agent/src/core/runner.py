...
# The only paths under ~/.vmx re-exposed into the ephemeral sandbox
...
def _prepare_sandbox_home(real_home: Path | None) -> Path:
    ...
    the ephemeral home re-exposes the narrow set of ``~/.vmx`` paths the
    ...
    if real_home is not None:
        src_root = real_home / ".vmx"
        if src_root.is_dir():
            dst_root = sandbox / ".vmx"
...
class Runner:
    ...
    def execute(
        ...
        *,
        ...
        # HOME (so it can't read the persistent ~/.vmx secrets/state),
