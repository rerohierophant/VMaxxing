...
```bash
VIBE_TRADING_ENABLE_SCHEDULER=1 vmx serve --port 8899
```
...
The API returns a job record with `id`, `status`, `next_run_at`, and the full schedule definition. Jobs persist under `~/.vmx/` and survive server restarts.
