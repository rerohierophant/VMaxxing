...
This reference is based on the frozen VMaxxing QVeris contract dated
...
`QVERIS_BASE_URL`; VMaxxing's default is the production API v1 endpoint from
the frozen contract.
...
| `session_id` | string | no | Optional VMaxxing session id. |
...
Executes one capability. VMaxxing must keep QVeris unavailable in `free`
...
| `session_id` | string | no | Correlates execution to VMaxxing session. |
...
VMaxxing clients should use a minimum request interval of 0.5 seconds and at
