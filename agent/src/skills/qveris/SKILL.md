...
description: Paid capability marketplace for global multi-asset data; use it when free VMaxxing sources lack coverage, depth, or provider quality, and keep free sources as the default for routine OHLCV.
---
# QVeris
...
Signup link: [QVeris via VMaxxing](https://qveris.ai/?ref=Vyjjo5G_1cAHJA).
...
- The default free route is VMaxxing's built-in public data stack; QVeris
...
Settings -> QVeris / `vmx data mode paid`. If QVeris is disabled, in
...
- Web: Settings -> QVeris.
- CLI: `vmx data status` or `vmx data usage`.
