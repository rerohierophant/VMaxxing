---
name: vnpy-export
description: Export a VMaxxing backtest strategy to a runnable vnpy CtaTemplate Python class — supports A-share equities, futures, and crypto via BarGenerator + ArrayManager.
category: tool
---

## Overview

This skill translates a VMaxxing strategy into a **vnpy `CtaTemplate` subclass** `.py` file
...
run a VMaxxing strategy inside vnpy's CTA backtester or live trading engine.
...
| Python (VMaxxing / pandas / ta-lib) | vnpy ArrayManager |
|----------------------------------------|-------------------|
...
| VMaxxing signal | Position check | vnpy call |
|---------------------|---------------|-----------|
...
When the VMaxxing strategy uses multiple timeframes (e.g., daily signal, hourly entry):
...
- [ ] Comment block at top of file notes the original VMaxxing `run_id` and instrument
