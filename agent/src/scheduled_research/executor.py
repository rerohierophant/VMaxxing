...
NotifyCallback = Callable[[ScheduledResearchJob, str, str], Awaitable[None]]
...
def is_due(job: ScheduledResearchJob, now_ms: int) -> bool:
    ...
    """
    if job.status in {JobStatus.CANCELLED, JobStatus.RUNNING, JobStatus.FAILED, JobStatus.PAUSED}:
        return False
...
class ScheduledResearchExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            notify: Optional async callable invoked after each job completes
                or fails. Receives ``(job, status, summary)`` where *status*
                is ``"completed"`` or ``"failed"`` and *summary* is a
    ...
    async def _run_job(self, job: ScheduledResearchJob, now_ms: int) -> None:
        ...
        # Notify after dispatch completes or fails (best-effort, never blocks
        # the scheduler tick).
        if self._notify is not None:
            status_label = final_status.value
            summary = f"Job '{job.id}' {status_label}: {job.prompt[:200]}"
            if dispatch_error:
                summary += f" (error: {dispatch_error})"
            try:
                await self._notify(job, status_label, summary)
            except Exception:
                logger.error("notification callback failed for job %s", job.id, exc_info=True)
