...
the Futu login, and the SDK speaks to it over a local socket. VMaxxing never
...
def build_config(profile_config: Mapping[str, Any] | None = None, overrides: Mapping[str, Any] | None = None) -> "FutuConfig":
    ...
    ``~/.vmx/futu.json``; the selected connector profile supplies the
...
def load_config() -> FutuConfig:
    """Load Futu settings from ``~/.vmx/futu.json``."""
    path = config_path()
    if not path.exists():
        return FutuConfig()
    try:
