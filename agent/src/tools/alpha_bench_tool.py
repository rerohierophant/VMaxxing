...
Cache integrity note: the universe panel cache lives in ``~/.vmx/cache/``
...
def _load_universe_panel(
    ...
    Args:
        ...
        use_cache: When True (default) reuse a pickle in
            ``~/.vmx/cache/`` if the same universe+period was fetched
...
def _fetch_sp500_constituents() -> list[str]:
    ...
    try:
        ...
        resp = requests.get(
            ...
            headers={
                "User-Agent": (
                    "VMaxxing/0.1 (research bench; "
                    "https://github.com/HKUDS/VMaxxing)"
                )
            },
            timeout=20,
        )
...
class AlphaBenchTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "output_dir": {
                "type": "string",
                "description": "Where to write the HTML report; default ~/.vmx/reports/.",
            },
        },
