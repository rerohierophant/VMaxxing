...
VMaxxing. It does not replace `CONTRIBUTING.md`, `SECURITY.md`, or the pull
...
- MCP entry point: `vmx-mcp` / `agent/mcp_server.py`.
- CLI entry point: `vmx` / `agent/cli/`.
...
- write real credentials into `agent/.env`, `~/.vmx/`, token caches, or
  external services,
